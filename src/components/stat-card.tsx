...
import { useIsSnapshot } from "@/lib/snapshot-context"
...
function useCountUp(value: string, duration = 1200) {
  const skip = useIsSnapshot()
  const ref = useRef<HTMLDivElement>(null)
  const parsed = useMemo(() => parseValue(value), [value])
  const [animated, setAnimated] = useState(() => formatValue(parsed, 0))
  // In snapshot mode, skip the animation entirely — show the final value on
  // mount. PDFs are static, and this lets the Puppeteer snap happen instantly
  // without waiting for the count-up to finish.
  const display = !parsed || skip ? value : animated
