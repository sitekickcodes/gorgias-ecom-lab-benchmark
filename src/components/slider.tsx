    ...
    >
      <SliderPrimitive.Control className="relative flex h-6 w-full touch-none items-center select-none data-[disabled]:opacity-50">
        ...
        {Array.from({ length: _values.length }, (_, index) => (
          ...
          >
            {formatValue && (
              // Inline `transform: translateX(-50%)` instead of Tailwind's
              // `-translate-x-1/2`. Tailwind v4's translate utility compiles
              // to a `--tw-translate-*` / `@property` chain that silently
              // fails inside shadow roots (Chromium doesn't honor @property
              // scoped to a shadow root). Direct CSS avoids the issue.
              <span
                style={{ transform: "translateX(-50%)" }}
                className="pointer-events-none absolute bottom-full left-1/2 mb-2.5 whitespace-nowrap rounded-lg bg-[#292827] px-2 py-1 font-mono text-xs text-white opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100 group-active:opacity-100 group-data-[dragging]:opacity-100"
              >
