...
interface BenchmarkProviderProps {
  ...
  /** Initial industry (e.g. from URL for snapshot pages). Defaults to "All Industries". */
  initialIndustry?: string
  /** Initial dataset. Defaults to "gmv". */
...
export function BenchmarkProvider({
  ...
  const [gmvSlider, setGmvSlider] = useState(
    initialDataset === "gmv" && initialSliderValue != null
      ? initialSliderValue
      : 50,
  )
  const [autoSlider, setAutoSlider] = useState(
    initialDataset === "automation-rate" && initialSliderValue != null
      ? initialSliderValue
      : 25,
  )
