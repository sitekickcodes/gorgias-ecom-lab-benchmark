...
/** Shared fields present in both tables */
const SHARED_FIELD_MAP: Record<string, string> = {
  Industry: "industry",
...
/** GMV table has "Approximated Gmv" → "tier" */
const GMV_FIELD_MAP: Record<string, string> = {
  ...SHARED_FIELD_MAP,
  "Approximated Gmv": "tier",
}

/** Automation rate table has "Automation Rate Pct" → "tier" */
...
async function fetchTable(tableId: string, fieldMap: Record<string, string>) {
...
export default async function handler(
  ...
  try {
    const [gmv, auto] = await Promise.all([
      fetchTable(GMV_TABLE_ID, GMV_FIELD_MAP),
      fetchTable(AUTO_TABLE_ID, AUTO_FIELD_MAP),
    ])
    res.setHeader(
      "Cache-Control",
